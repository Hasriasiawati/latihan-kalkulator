 <html>

<head>

                <title>Membuat Kalkulator Sederhana Menggunakan PHP</title>

                <link rel="stylesheet" type="text/css" href="style.css">

</head>

<body>

                <?php

// function isset() disini berfungsi untuk mengecek data, jadi apabila form di submit maka data akan tersimpan di masing-masing variabel.

                if(isset($_POST['hitung'])){

                                 // Variabel

                                $txt1 = $_POST['txt1'];

                                $txt2 = $_POST['txt2'];

                                $operator = $_POST['operator'];

                                                //untuk fungsi Switch case di sini untuk mengecek operator apa yang pilih sebelum data di submit.

                                                switch ($operator) {

                                                                case 'tambah':

                                                                                $hasil = $txt1+$txt2;

                                                                break;

                                                                case 'kurang':

                                                                                $hasil = $txt1-$txt2;

                                                                break;

                                                                case 'kali':

                                                                                $hasil = $txt1*$txt2;

                                                                break;

                                                                case 'bagi':

                                                                                $hasil = $txt1/$txt2;

                                                                break;                                  

                                                }

                }

                ?>

                <div class="kalkulator">

                                <h1 class="judul">KALKULATOR</h1>

                                <form method="post" action="index2.php">                                      

                                                <input type="text" name="txt1" class="txt" placeholder="Type Here ...">

                                                <input type="text" name="txt2" class="txt" placeholder="Type Here ...">

                                                <select class="operator" name="operator">

                                                                <option value="tambah"> + </option>

                                                                <option value="kurang"> - </option>

                                                                <option value="kali"> * </option>

                                                                <option value="bagi"> / </option>

                                                </select>

                                                <input type="submit" name="hitung" value="HITUNG" class="tombol">

                                                <input type="submit" name="reset" value="RESET" class="tombol">                                                                                                                                                                    

                                </form>
                                <?php
                                
//author : https://gist.github.com/cahsowan/d315d54a59e4f14a6bab
function terbilang($hasil) {
  $angka = ["nol", "satu", "dua", "tiga", "empat", "lima", "enam", "tujuh", "delapan", "sembilan", "sepuluh", "sebelas"];
  if ($hasil < 12)
    return " " . $angka[$hasil];
  elseif ($hasil < 20)
    return terbilang($hasil - 10) . " belas";
  elseif ($hasil < 100)
    return terbilang($hasil / 10) . " puluh" . terbilang($hasil % 10);
  elseif ($hasil < 200)
    return "seratus" . terbilang($hasil - 100);
  elseif ($hasil < 1000)
    return terbilang($hasil / 100) . " ratus" . terbilang($hasil % 100);
  elseif ($hasil < 2000)
    return "seribu" . terbilang($hasil - 1000);
  elseif ($hasil < 1000000)
    return terbilang($hasil / 1000) . " ribu" . terbilang($hasil % 1000);
  elseif ($hasil < 1000000000)
    return terbilang($hasil / 1000000) . " juta" . terbilang($hasil % 1000000);
$terbilang = terbilang($hasil);
  
}
?>
                                <?php if(isset($_POST['hitung'])){ ?>

                                                <input type="text" value="<?php echo $hasil; ?>" class="txt">
                                                <input type="text" value="<?php echo $terbilang; ?>" class="txt">

                                <?php }else{ ?>

                                <?php } ?>                                         

                </div>

</body>

 

</html>  